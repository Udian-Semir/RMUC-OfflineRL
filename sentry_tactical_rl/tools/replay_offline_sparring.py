...
def main() -> None:
    ...
    frames.append((
        ...
        building_hp(env), {"hp": float(env.sentry.hp)}, env.blue_sparring_profile,
    ))
    ...
    while not done:
        ...
        frames.append((
            ...
            {"hp": float(env.sentry.hp)}, env.blue_sparring_profile,
        ))
    ...
    def render_frame(frame: tuple[int, list[Unit], dict[int, Any], tuple[dict[str, Any], ...], dict[str, float], dict[str, float], str]) -> Image.Image:
        ...
        return render_replay_frame(
            ...
            detail=f"PPO sentry hp={sentry_state['hp']:.0f} | rings: vehicle 3m/5m, hero 6.5m, sentry 8m",
        )
