...
当前是纯 Python 的低保真 2D 战术环境，用于验证接口和训练链路。`SemanticMap.demo()` 是**合成地图**，不是官方 RMUC 场地；在使用训练结果前必须替换为实测地图、哨兵可通行掩码和校准参数。
...
暂未实现：正式场地地图、真实裁判伤害与资源规则、雷达输入桥接、ROS2 发布节点、BC/IQL/DT 陪练适配和实车标定。现有离线模型不能直接塞进本环境：它们依赖 161 维裁判日志观测，必须先定义“仿真状态/雷达状态 → 该观测”的一致转换，避免无意引入全知信息。
