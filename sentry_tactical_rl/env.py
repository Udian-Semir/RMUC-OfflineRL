...
class SentryTacticalEnv:
        ...
        for team in ("red", "blue"):
            for role in ROLE_SPECS:
                if team == "red" and role == "sentry":
                    continue
                configured = Path(self.sparring_run_dirs[role])
                if configured.is_absolute():
                    run_dir = configured
                else:
                    direct = repository_root / configured
                    run_dir = direct if direct.exists() else repository_root / "rm_runs" / configured
                ...
                if key not in self._sparring_policies:
                    self._sparring_policies[key] = OfflineSparringPolicy(
                        str(run_dir), semantic_map=self.map, team=team, role=role, device=self.sparring_device)
                self._sparring_policy_keys[(team, role)] = key

    def _sparring_policy_for(self, unit: Unit) -> Any:
        try:
            return self._sparring_policies[self._sparring_policy_keys[(unit.team, unit.role)]]
    ...
    def step(self, action: tuple[int, int, int] | list[int] | np.ndarray) -> tuple[dict[str, np.ndarray], float, bool, dict[str, Any]]:
        ...
        requested_goal_idx, target_idx, fire_mode = self._scheduled_sentry_action(
            requested_goal_idx, target_idx, fire_mode)
        self._last_fire_result = {"robot": 0.0, "blue_outpost": 0.0, "blue_base": 0.0}
        self._combat_events = []
        ...
        healthy_hunt = self._healthy_hunt_phase()
        ...
        elif 0 <= goal_idx < self.n_goals:
            selected_anchor = self.map.anchors[self.anchor_names[goal_idx]]
            goal = self._sentry_execution_goal(selected_anchor, target_idx)
        ...
        if (not external_sentry and self.sentry.alive and fire_mode == self.FIRE_ENGAGE
    ...
    def _update_sentry_recovery_state(self) -> bool:
        ...
        if not self.sentry.alive:
            ...
            if self.sentry.hp >= self.sentry.max_hp * 0.90:
                self._sentry_recovery_active = False
        elif self.sentry.hp <= self.sentry.max_hp * 0.25:
            self._sentry_recovery_active = True
    ...
    def _healthy_hunt_phase(self) -> bool:
        return (
            self.sentry.alive
            and self.sentry.hp >= self.sentry.max_hp * 0.60
    ...
    def _sentry_execution_goal(self, learned_goal: Cell, target_idx: int) -> Cell:
        ...
        if target is None or not self._healthy_hunt_phase():
            return learned_goal
    ...
    def _sentry_fire(self, target_idx: int) -> float:
        ...
        if distance > self.sentry.attack_range or not self.map.line_of_sight(self.sentry.cell, target_cell):
            return 0.0
    ...
    def _move_sparring_unit(self, unit: Unit, goal: Cell) -> None:
        ...
        if unit.role == "aerial":
            ...
            return
        nav = self.navigator.plan(unit.cell, goal, self._team_threat_layer(unit.team))
        if nav.reachable:
    ...
    def _attack_standoff_goal(self, unit: Unit, target_cell: Cell, fallback: Cell) -> Cell:
        ...
        for y in range(max(0, target_cell[1] - radius_cells),
            ...
            for x in range(max(0, target_cell[0] - radius_cells),
                ...
                cell = (x, y)
                if not self.map.is_free(cell) or cell == target_cell:
                    continue
                distance = self._distance(cell, target_cell)
                if distance < 0.75 or distance > unit.attack_range:
                    continue
                if not self.map.line_of_sight(cell, target_cell):
                    continue
    ...
    def _can_hit_unit(self, attacker: Unit, target: Unit) -> bool:
        return (
            ...
            and self.map.line_of_sight(attacker.cell, target.cell)
        )
    ...
    def _fire_unit_at_structure(
        ...
        *,
        ...
        if distance > attacker.attack_range or not self.map.line_of_sight(attacker.cell, target_cell):
            return
