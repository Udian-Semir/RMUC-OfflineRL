...
# ``rm_rl`` is a sibling package in the RMUC-OfflineRL source tree rather than
# a separately installed distribution.  ROS 2 starts console scripts with the
# package install path only, so make the sibling importable when this package
# is run from that source tree.  A future standalone deployment can vendor or
# install ``rm_rl`` and will simply skip this branch.
for _parent in Path(__file__).resolve().parents:
    if (_parent / "rm_rl").is_dir():
