...
class RadarCostMap:
    ...
    def from_ros_map_yaml(
        ...
        *,
        ...
        inflation_cells = sentry_radius_m / target_resolution_m
        hard = inflate_mask(hard, inflation_cells)
        proximity_cells = proximity_m / target_resolution_m
        near_obstacle = inflate_mask(hard, proximity_cells)
...
def inflate_mask(mask: np.ndarray, radius: float) -> np.ndarray:
    """Euclidean binary dilation with a fractional radius in grid cells."""
    if radius <= 0:
        return mask.copy()
    h, w = mask.shape
    out = mask.copy()
...
# Kept for internal callers and older imports while the public helper carries
# the unit-bearing name used by the semantic tactical map.
_dilate = inflate_mask
