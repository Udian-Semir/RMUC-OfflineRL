...
def main() -> None:
    ...
    parser.add_argument("--reset-optimizer", action="store_true",
                        help="load only policy/value weights from --resume and start a fresh optimizer")
    ...
    trainer = PPOTrainer(env, ppo, device=_device(str(train_cfg.get("device", "auto"))), seed=seed)
    start_update = 0
    if args.resume:
        metadata = trainer.load(args.resume, load_optimizer=not args.reset_optimizer)
        start_update = int(metadata.get("update", 0))
        optimizer_mode = "reset" if args.reset_optimizer else "restored"
        print(
            f"resumed checkpoint={args.resume}; previous_update={start_update}; "
    ...
    print(
        f"training on {trainer.device}; envs={trainer.num_envs}; "
        f"steps_per_env={ppo.rollout_steps}; samples_per_update={trainer.num_envs * ppo.rollout_steps}; "
        f"goals={env.n_goals}, targets={env.n_targets}, vector={env.vector_dim}"
    )
